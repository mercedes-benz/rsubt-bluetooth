...
public class A2dpService extends ProfileService {
    ...
    protected boolean stop() {
        ...
        // duplicate it
        List<BluetoothDevice> activeDevices = new LinkedList<>();
        for (BluetoothDevice device : mActiveDevices)  {
    ...
    private void removeActiveDevice(BluetoothDevice device, boolean forceStopPlayingAudio) {
        ...
        synchronized (mActiveSwitchingGuard) {
            ...
            synchronized (mStateMachines) {
                if (!mActiveDevices.contains(device)) return;
                mActiveDevices.remove(device);
                updateAndBroadcastActiveDevice(device, false);
    ...
    public boolean setActiveDevice(BluetoothDevice device, boolean active) {
        synchronized (mActiveSwitchingGuard) {
            ...
            synchronized (mStateMachines) {
                ...
                if (active && sm.getConnectionState() != BluetoothProfile.STATE_CONNECTED) {
            ...
            synchronized (mStateMachines) {
                if (!mA2dpNativeInterface.setActiveDevice(device, active)) {
                    Log.e(TAG, "setActiveDevice(" + device + ", " + active + "): Cannot set as active in native "
                        + "layer");
                    return false;
                }
    ...
    private int getAudioZoneIndex(BluetoothDevice device, String mediaPlayer) {
        ...
        if (oldMediaPlayer.isEmpty()) {
            audioZoneIndex = A2dpAudioZone.getAudioZoneAvailable(device);
        } else {
    ...
    // The media player meapped a headset audio zone
    BluetoothDevice getMediaPlayerMappedDevice(String mediaPlayer) {
    ...
    public boolean setMediaPlayer(BluetoothDevice device, String mediaPlayer) {
        ...
        synchronized (mStateMachines) {
            ...
            // Don't save media player for current device if media player has mapped with other device
            if ((getMediaPlayerMappedDevice(mediaPlayer) == null) &&
                !mDatabaseManager.setA2dpMediaPlayer(device, mediaPlayer, audioZoneIndex)) {
                return false;
            }
    ...
    static class BluetoothA2dpBinder extends IBluetoothA2dp.Stub
        ...
        @Override
        public void getMediaPlayer(BluetoothDevice device, AttributionSource source,
                SynchronousResultReceiver receiver) {
            try {
                A2dpService service = getService(source);
                String result = null;
                if (service != null) {
                    result = service.getMediaPlayer(device);
                }
                receiver.send(result);
            } catch (RuntimeException e) {
                receiver.propagateException(e);
        ...
        @Override
        public void setMediaPlayer(BluetoothDevice device, String mediaPlayer, AttributionSource source,
                SynchronousResultReceiver receiver) {
            try {
                ...
                boolean result = false;
                if (service != null) {
                    result = service.setMediaPlayer(device, mediaPlayer);
                }
                receiver.send(result);
            } catch (RuntimeException e) {
                receiver.propagateException(e);
