...
// APRICOT-382703: implement function for AVRCP. @{
static void sendMediaKeyEventWithAddress(int key, KeyState keyState, const RawAddress& address);
// @}
static std::string getCurrentMediaId();
static std::string getCurrentMediaIdExt(const RawAddress& address);
static PlayStatus getCurrentPlayStatusWithAddress(const RawAddress& address);
...
static void sendDelayReportEventWithAddress(const RawAddress& address, uint16_t delay_report);
...
static void setVolumeWithAddress(int8_t volume, const RawAddress& address);
 ...
 public:
  void SendKeyEvent(uint8_t key, KeyState state) {
   ...
   void SendKeyEventExt(const RawAddress& address, uint8_t key, KeyState state) {
    ...
    sendMediaKeyEventWithAddress(key, state, address);
   }
  // APRICOT-382703: implement function for AVRCP. @{
  void SendKeyEventWithAddress(uint8_t key, KeyState state, const RawAddress& address) {
    sendMediaKeyEventWithAddress(key, state, address);
  }
  // @}
  ...
  void GetPlayStatusExt(const RawAddress& address, PlayStatusCallback cb) override {
    auto status = getCurrentPlayStatusWithAddress(address);
    cb.Run(status);
  }
  void GetPlayStatusWithAddress(PlayStatusWithAddressCallback cb, const RawAddress& address) override {
    auto status = getCurrentPlayStatusWithAddress(address);
    cb.Run(status, address);
  }
  ...
  void GetNowPlayingListExt(const RawAddress& address, NowPlayingCallback cb) override {
    ALOGD("%s %s", __func__, address.ToString().c_str());
  ...
  void SendDelayReportEventWithAddress(const RawAddress& address, uint16_t delay_report) {
    sendDelayReportEventWithAddress(address, delay_report);
  }
...
class VolumeInterfaceImpl : public VolumeInterface {
 public:
  ...
  void SetVolumeExt(const RawAddress& bdaddr, int8_t volume) override { setVolumeWithAddress(volume, bdaddr); }
  void SetVolumeWithAddress(int8_t volume, const RawAddress& address) override { setVolumeWithAddress(volume, address); }
};
...
static jmethodID method_getPlaybackStatusWithAddress;
// static jmethodID method_getPlaybackStatusExt;
static jmethodID method_sendMediaKeyEvent;
// APRICOT-382703: implement function for AVRCP. @{
static jmethodID method_sendMediaKeyEventWithAddress;
// @}
...
static jmethodID method_sendDelayReportEventWithAddress;
...
static jmethodID method_setVolumeWithAddress;
...
static void classInitNative(JNIEnv* env, jclass clazz) {
  ...
  // method_getCurrentSongInfoExt = env->GetMethodID(
  //     clazz, "getCurrentSongInfoExt", "(Ljava/lang/String;)Lcom/android/bluetooth/audio_util/Metadata;");

  method_getPlaybackStatus = env->GetMethodID(
      clazz, "getPlayStatus", "()Lcom/android/bluetooth/audio_util/PlayStatus;");

  method_getPlaybackStatusWithAddress = env->GetMethodID(
      clazz, "getPlayStatusWithAddress", "(Ljava/lang/String;)Lcom/android/bluetooth/audio_util/PlayStatus;");

  // method_getPlaybackStatusExt = env->GetMethodID(
  //     clazz, "getPlayStatusExt", "(Ljava/lang/String;)Lcom/android/bluetooth/audio_util/PlayStatus;");

  method_sendMediaKeyEvent =
      env->GetMethodID(clazz, "sendMediaKeyEvent", "(IZ)V");
  // APRICOT-382703: implement function for AVRCP. @{
  method_sendMediaKeyEventWithAddress =
      env->GetMethodID(clazz, "sendMediaKeyEventWithAddress", "(IZLjava/lang/String;)V");
  // @}

  // method_sendMediaKeyEventExt =
  //     env->GetMethodID(clazz, "sendMediaKeyEventExt", "(Ljava/lang/String;IZ)V");

  method_getCurrentMediaId =
      env->GetMethodID(clazz, "getCurrentMediaId", "()Ljava/lang/String;");

  // method_getCurrentMediaIdExt =
  //     env->GetMethodID(clazz, "getCurrentMediaIdExt", "(Ljava/lang/String;)Ljava/lang/String;");

  method_getNowPlayingList =
      env->GetMethodID(clazz, "getNowPlayingList", "()Ljava/util/List;");

  // method_getNowPlayingListExt =
  //     env->GetMethodID(clazz, "getNowPlayingListExt", "(Ljava/lang/String;)Ljava/util/List;");
  ...
  method_sendDelayReportEventWithAddress =
      env->GetMethodID(clazz, "sendDelayReportEventWithAddress", "(Ljava/lang/String;I)V");
  ...
  method_setVolumeWithAddress = env->GetMethodID(clazz, "setVolumeWithAddress", "(ILjava/lang/String;)V");


  // method_setVolumeExt = env->GetMethodID(clazz, "setVolumeExt", "(Ljava/lang/String;I)V");
...
static void sendMediaUpdateWithAddressNative(JNIEnv* env, jobject object,
                                  jboolean metadata, jboolean state,
                                  jboolean queue, jstring address) {
  ALOGD("%s", __func__);
  std::unique_lock<std::shared_timed_mutex> interface_lock(interface_mutex);
  if (mServiceCallbacks == nullptr) {
    ALOGW("%s: Service not loaded.", __func__);
...
static int getDelayReportWithAddressNative(JNIEnv* env, jobject object, jstring address) {
  ALOGD("%s", __func__);
  std::unique_lock<std::shared_timed_mutex> interface_lock(interface_mutex);
  if (mServiceCallbacks == nullptr) {
    ALOGW("%s: Service not loaded.", __func__);
  ...
  if (!success) return 0;
  // return mServiceCallbacks->GetDelayReportWithAddress(bdaddr);
  return 0;
}
...
static PlayStatus getCurrentPlayStatusWithAddress(const RawAddress& address) {
  ...
  jstring j_bdaddr = sCallbackEnv->NewStringUTF(address.ToString().c_str());
  jobject playStatus =
      sCallbackEnv->CallObjectMethod(mJavaInterface, method_getPlaybackStatusWithAddress, j_bdaddr);
...
static void sendDelayReportEventWithAddress(const RawAddress& address, uint16_t delay_report) {
  ALOGD("%s addr:%s delay_report:%d", __func__, address.ToString().c_str(), delay_report);
  std::shared_lock<std::shared_timed_mutex> lock(callbacks_mutex);
  CallbackEnv sCallbackEnv(__func__);
  if (!sCallbackEnv.valid() || !mJavaInterface) return;

  jstring j_bdaddr = sCallbackEnv->NewStringUTF(address.ToString().c_str());
  sCallbackEnv->CallVoidMethod(mJavaInterface, method_sendDelayReportEventWithAddress,
                               j_bdaddr, delay_report);
}
...
static void setVolumeWithAddress(int8_t volume, const RawAddress& address) {
  ...
  jstring j_bdaddr = sCallbackEnv->NewStringUTF(address.ToString().c_str());
  sCallbackEnv->CallVoidMethod(mJavaInterface, method_setVolumeWithAddress, volume, j_bdaddr);
}
...
static JNINativeMethod sMethods[] = {
    ...
    {"sendMediaUpdateWithAddressNative", "(ZZZLjava/lang/String;)V", (void*)sendMediaUpdateWithAddressNative},
    {"getDelayReportWithAddressNative", "(Ljava/lang/String;)I", (void*)getDelayReportWithAddressNative},
...
// APRICOT-382703: implement function for AVRCP. @{
void sendMediaKeyEventWithAddress(int key, KeyState state,
                                  const RawAddress& address) {
  ALOGD("%s", __func__);
  std::shared_lock<std::shared_timed_mutex> lock(callbacks_mutex);
  CallbackEnv sCallbackEnv(__func__);
  ...
  jstring j_bdaddr = sCallbackEnv->NewStringUTF(address.ToString().c_str());
  sCallbackEnv->CallVoidMethod(
      mJavaInterface, method_sendMediaKeyEventWithAddress, key,
      state == KeyState::PUSHED ? JNI_TRUE : JNI_FALSE, j_bdaddr);
