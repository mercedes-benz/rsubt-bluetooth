...
static void bta2dp_connection_state_callback(const RawAddress& bd_addr,
  ...
  if (state == BTAV_CONNECTION_STATE_CONNECTED) {
    ALOGI("%s: calling set_active_device(true) for connected device", __func__);
    bt_status_t status = sBluetoothA2dpInterface->set_active_device(bd_addr, true);
    if (status != BT_STATUS_SUCCESS) {
      ALOGE("%s: Failed to enable active device, status: %d", __func__, status);
    }
  } else if (state == BTAV_CONNECTION_STATE_DISCONNECTED) {
    ALOGI("%s: calling set_active_device(false) for disconnected device", __func__);
    bt_status_t status = sBluetoothA2dpInterface->set_active_device(bd_addr, false);
    if (status != BT_STATUS_SUCCESS) {
      ALOGE("%s: Failed to disable active device, status: %d", __func__, status);
    }
  }
}
...
static jboolean setActiveDeviceNative(JNIEnv* env, jobject object,
                                      jbyteArray address, jint displayId) {
  ALOGI("%s: sBluetoothA2dpInterface: %p", __func__, sBluetoothA2dpInterface);
  std::shared_lock<std::shared_timed_mutex> lock(interface_mutex);
  if (!sBluetoothA2dpInterface) {
    ALOGE("%s: Failed to get the Bluetooth A2DP Interface", __func__);
