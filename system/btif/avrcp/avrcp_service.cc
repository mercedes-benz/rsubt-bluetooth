...
class MediaInterfaceWrapper : public MediaInterface {
 public:
  ...
  void SendKeyEventWithAddress(uint8_t key, KeyState state, const RawAddress& address) override {
      LOG(INFO) << __func__ << " key=%d" << key;
    do_in_avrcp_jni(base::Bind(&MediaInterface::SendKeyEventWithAddress,
                               base::Unretained(wrapped_), key, state, address));
    }
  ...
  void GetPlayStatusWithAddress(PlayStatusWithAddressCallback status_cb, const RawAddress& address) override {
    auto cb_lambda = [](PlayStatusWithAddressCallback cb, PlayStatus status, const RawAddress& address) {
      do_in_main_thread(FROM_HERE, base::Bind(cb, status, address));
    };

    auto bound_cb = base::Bind(cb_lambda, status_cb);

    do_in_avrcp_jni(base::Bind(&MediaInterface::GetPlayStatusWithAddress,
                               base::Unretained(wrapped_), bound_cb, address));
  }
  ...
  void SendDelayReportEventWithAddress(const RawAddress& address, uint16_t delay_report) override {
    do_in_avrcp_jni(base::Bind(&MediaInterface::SendDelayReportEventWithAddress,
                               base::Unretained(wrapped_), address, delay_report));
  }
...
class VolumeInterfaceWrapper : public VolumeInterface {
 public:
  ...
  void SetVolumeWithAddress(int8_t volume, const RawAddress& bdaddr) override {
    do_in_avrcp_jni(base::Bind(&VolumeInterface::SetVolumeWithAddress,
                               base::Unretained(wrapped_), volume, bdaddr));
  }
...
void AvrcpService::SendMediaUpdateWithAddress(bool track_changed, bool play_state,
                       bool queue, const RawAddress& address) {
  // APRICOT-411766
}
...
uint16_t AvrcpService::GetDelayReportWithAddress(const RawAddress& address) {
  // APRICOT-429072
  return 0;
}
