...
static std::map<int, RawAddress> active_device_addr_by_display_id;
...
/**
 * Check if bd_addr is the active device for its ACL display.
 *
 * @param bd_addr target device address
 * @return True if bd_addr is active on the display associated with its ACL.
 */
bool bta_ag_sco_is_active_device(const RawAddress& bd_addr) {
  tACL_CONN* p_acl = btm_acl_for_bda(bd_addr, BT_TRANSPORT_BR_EDR);
  if (p_acl == nullptr) {
    return false;
  }
  const auto it = active_device_addr_by_display_id.find(p_acl->display_id);
  return it != active_device_addr_by_display_id.end() &&
         it->second == bd_addr;
}
  ...
  if (event == BTM_ESCO_CONN_REQ_EVT) {
    ...
    if (remote_bda && bta_ag_sco_is_active_device(*remote_bda) && p_scb &&
      ...
      LOG(WARNING) << __func__
                   ...
                   << ", active_count=" << active_device_addr_by_display_id.size()
                   << ", current_bda="
...
static void bta_ag_create_sco(tBTA_AG_SCB* p_scb, bool is_orig) {
  ...
  if (!bta_ag_sco_is_active_device(p_scb->peer_addr)) {
    LOG(WARNING) << __func__ << ": device " << p_scb->peer_addr
                 << " is not active, active_count="
                 << active_device_addr_by_display_id.size();
    if (bta_ag_cb.sco.p_curr_scb != nullptr &&
...
bool bta_ag_has_active_device(const RawAddress& bd_addr) {
  for (const auto& [display_id, active_device_addr] :
       active_device_addr_by_display_id) {
    (void)display_id;
    if (active_device_addr == bd_addr) {
      return true;
    }
  }
  return false;
...
void bta_clear_active_device(const RawAddress& bd_addr) {
  for (auto it = active_device_addr_by_display_id.begin();
       it != active_device_addr_by_display_id.end();) {
    if (it->second == bd_addr) {
      it = active_device_addr_by_display_id.erase(it);
    } else {
      ++it;
    }
  }
...
void bta_ag_api_set_active_device(const RawAddress& new_active_device,
                                  int display_id) {
  LOG_INFO("bta_ag_api_set_active_device: new_active_device: %s, display_id: %d", new_active_device.ToString().c_str(), display_id);
  if (new_active_device.IsEmpty()) {
    APPL_TRACE_ERROR("%s: empty device", __func__);
    return;
  }
  active_device_addr_by_display_id[display_id] = new_active_device;
  acl_set_display_id(new_active_device, display_id);
}
