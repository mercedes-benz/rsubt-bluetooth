...
static void btif_a2dp_data_cb([[maybe_unused]] tUIPC_CH_ID ch_id,
  ...
  switch (event) {
    ...
    case UIPC_CLOSE_EVT:
       ...
       * abnormally.
       */
      if (btif_a2dp_source_is_streaming(RawAddress::kEmpty)) {
...
static void a2dp_data_path_open() {
  UIPC_Open(RawAddress::kEmpty, *a2dp_uipc, UIPC_CH_ID_AV_AUDIO, btif_a2dp_data_cb,
            A2DP_HOST_DATA_PATH);
...
bool init(bluetooth::common::MessageLoopThread* message_loop) {
  LOG_INFO("%s: message_loop=%p", __func__, message_loop);
  a2dp_uipc = UIPC_Init();
  total_bytes_read_ = 0;
  data_position_ = {};
