...
BluetoothAudioCtrlAck A2dpTransport::StartRequest(bool is_low_latency) {
  ...
  if (btif_av_stream_ready(RawAddress::kEmpty)) {
    ...
    btif_av_stream_start_with_latency(RawAddress::kEmpty, is_low_latency);
...
bool init(bluetooth::common::MessageLoopThread* message_loop) {
  ...
  active_hal_interface =
      (offloading_hal_interface != nullptr ? offloading_hal_interface
                                           : software_hal_interface);
  LOG(INFO) << __func__ << ": offloading_hal_interface=" << offloading_hal_interface
            << ", software_hal_interface=" << software_hal_interface
            << ", active_hal_interface=" << active_hal_interface;
