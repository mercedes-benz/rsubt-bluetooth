...
void acl_set_display_id(const RawAddress& bd_addr, int display_id) {
  tACL_CONN* p_acl = btm_acl_for_bda(bd_addr, BT_TRANSPORT_BR_EDR);
  LOG_INFO("acl_set_display_id: bd_addr: %s, display_id: %d",
           bd_addr.ToString().c_str(), display_id);
  if (p_acl != nullptr) {
    p_acl->display_id = display_id;
  }
}
