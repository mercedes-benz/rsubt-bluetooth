...
static esco_data_path_t btm_sco_get_data_path(const RawAddress& bd_addr) {
  LOG_INFO("%s: btm_sco_get_data_path: bd_addr: %s",__func__, bd_addr.ToString().c_str());
  tACL_CONN* p_acl = btm_acl_for_bda(bd_addr, BT_TRANSPORT_BR_EDR);
  if (p_acl != nullptr && p_acl->display_id == 0) {
    LOG_INFO("%s: return ESCO_DATA_PATH", __func__);
    return ESCO_DATA_PATH;
  } else if (p_acl != nullptr && p_acl->display_id > 0) {
    LOG_INFO("%s: return ESCO_DATA_PATH_EXT", __func__);
    return ESCO_DATA_PATH_EXT;
  }
  return ESCO_DATA_PATH;
}
...
void btm_sco_connected(const RawAddress& bda, uint16_t hci_handle,
  ...
  LOG_INFO("btm_sco_connected: bda=%s, hci_handle=0x%04x", bda.ToString().c_str(), hci_handle);
  tSCO_CONN* p = &btm_cb.sco_cb.sco_db[0];
  uint16_t xx;
  bool spt = false;
...
static tBTM_STATUS BTM_ChangeEScoLinkParms(uint16_t sco_inx,
  ...
  } else /* eSCO is supported and the link type is eSCO */
  {
    ...
    if (controller_get_interface()
      ...
      /* Use the saved SCO routing */
      p_setup->input_data_path = p_setup->output_data_path =
          btm_sco_get_data_path(p_sco->esco.data.bd_addr);
