...
static const char* get_lib_name_by_address(const RawAddress& peer_address) {
  ...
  if (address_libs_map.find(peer_address) != address_libs_map.end()) {
    LOG_INFO("%s, address_libs_map is not empty", __func__);
    lib_name = address_libs_map.find(peer_address)->second;
    if (strcmp(lib_name, encoder_libs[0]) != 0 &&
          strcmp(lib_name, encoder_libs[1]) != 0) {
        LOG_ERROR("%s: unknown lib name %s", __func__, lib_name);
        return nullptr;
    }
    LOG_WARN("%s: lib %s has been used for current address", __func__, lib_name);
    return lib_name;
  }
  ...
  for (int i = 0; i < MAX_SBC_ENDOER_LIBS; i++) {
    ...
    for (auto it : address_libs_map) {
      ...
      if (strcmp(lib_name, encoder_libs[i]) == 0) {
          LOG_INFO("%s: lib name: %s has been used", __func__, lib_name);
          used = true;
          break;
      }
    }
    if (!used) {
      ...
      LOG_INFO("%s: find lib name: %s", __func__, lib_name);
      address_libs_map.emplace(peer_address, lib_name);
      return lib_name;
    }
  }
  LOG_ERROR("%s: No lib found", __func__);
  return nullptr;
}
...
void A2dpSbcEncoder::encoder_init(tA2DP_ENCODER_INIT_PEER_PARAMS* p_peer_params,
  ...
  LOG_INFO("%s, peer_address:%s",
            __func__, get_peer_address().ToString().c_str());
  ...
  LOG_INFO("%s: Open sbc encoder library %s", __func__, lib_name);
  sbc_encoder_lib_handle = dlopen(lib_name, RTLD_NOW);
  if (sbc_encoder_lib_handle == NULL) {
    LOG_ERROR("%s: cannot open sbc encoder library %s: %s", __func__,
              lib_name, dlerror());
    return;
  }
...
void A2dpSbcEncoder::a2dp_sbc_encoder_update(A2dpCodecConfig* a2dp_codec_config,
  ...
  *p_config_updated = false;
  ...
  if (sbcif == NULL) {
    LOG_ERROR("%s: cannot find symbol '%s' in the encoder library. Error: %s",
              __func__, SBC_ENCODE_INTERFACE_STRING, dlerror());
  ...
  LOG_INFO("%s: MTU=%d, peer_mtu=%d min_bitpool=%d max_bitpool=%d", __func__,
           this->TxAaMtuSize, peer_params.peer_mtu, min_bitpool,
           max_bitpool);
  LOG_INFO(
...
void A2dpSbcEncoder::feeding_reset(void) {
  ...
  memset(&this->feeding_state, 0,
         sizeof(this->feeding_state));
...
uint64_t A2dpSbcEncoder::get_encoder_interval_ms(void) {
  return A2DP_SBC_ENCODER_INTERVAL_MS;
}

int A2dpSbcEncoder::get_effective_frame_size() {
  return this->TxAaMtuSize;
}
...
void A2dpSbcEncoder::a2dp_sbc_get_num_frame_iteration(uint8_t* num_of_iterations,
  ...
  uint32_t pcm_bytes_per_frame =
      this->sbc_encoder_params.s16NumOfSubBands *
...
void A2dpSbcEncoder::a2dp_sbc_encode_frames(uint8_t nb_frame) {
  LOG_VERBOSE("%s: %s, nb_framel %d", __func__,
              get_peer_address().ToString().c_str(), nb_frame);
  SBC_ENC_PARAMS* p_encoder_params = &this->sbc_encoder_params;
  ...
  if (sbcif == NULL) {
    LOG_ERROR("%s: cannot find sysmbol '%s' in the encoder library, error: %s",
              __func__, SBC_ENCODE_INTERFACE_STRING, dlerror());
    dlclose(sbc_encoder_lib_handle);
    return ;
  }
  ...
  while (nb_frame) {
    ...
    do {
      ...
      if (a2dp_sbc_read_feeding(&num_bytes)) {
        ...
        ((p_buf->len + last_frame_len) < this->TxAaMtuSize) &&
    ...
    if (p_buf->len) {
      ...
      *((uint32_t*)(p_buf + 1)) = this->timestamp;
      ...
      this->timestamp += p_buf->layer_specific * blocm_x_subband;

      uint8_t done_nb_frame = remain_nb_frame - nb_frame;
      remain_nb_frame = nb_frame;
      if (!btif_a2dp_source_enqueue_callback(get_peer_address(),
                                                p_buf, done_nb_frame,
                                                bytes_read))
        return;
    } else {
...
bool A2dpSbcEncoder::a2dp_sbc_read_feeding(uint32_t* bytes_read) {
  ...
  if (sbc_sampling == this->feeding_params.sample_rate) {
    LOG_VERBOSE("%s: %s, sbc_sampling == sample_rate", __func__, get_peer_address().ToString().c_str());
    read_size =
        bytes_needed - this->feeding_state.aa_feed_residue;
    this->stats.media_read_total_expected_read_bytes += read_size;
    nb_byte_read = btif_a2dp_source_read_callback(
        get_peer_address(),
        ((uint8_t*)this->pcmBuffer) +
            this->feeding_state.aa_feed_residue,
        read_size);
    this->stats.media_read_total_actual_read_bytes +=
        nb_byte_read;

    *bytes_read = nb_byte_read;
    if (nb_byte_read != read_size) {
      LOG_WARN("%s: %s, nb_byte_read %d != read_size %d", __func__,
    ...
    this->feeding_state.aa_feed_residue = 0;
    LOG_VERBOSE("%s: %s, return true", __func__, get_peer_address().ToString().c_str());
    return true;
  }
  ...
  nb_byte_read =
      btif_a2dp_source_read_callback(get_peer_address(), (uint8_t*)read_buffer, read_size);
  ...
  if (nb_byte_read < read_size) {
    ...
    if (nb_byte_read == 0) {
      LOG_WARN("%s: %s, nb_byte_read is 0", __func__, get_peer_address().ToString().c_str());
      return false;
    }
  }
  this->stats.media_read_total_actual_reads_count++;
  ...
  mSbcUpSample->a2dp_sbc_init_up_sample(this->feeding_params.sample_rate,
                          sbc_sampling,
                          this->feeding_params.bits_per_sample,
                          this->feeding_params.channel_count);
  ...
  if (this->feeding_state.aa_feed_residue < bytes_needed) {
    LOG_WARN("%s: %s, aa_feed_residue:%d, bytes_needed:%d", __func__,
      get_peer_address().ToString().c_str(),
      this->feeding_state.aa_feed_residue, bytes_needed);
    return false;
  }

  /* Copy the output pcm samples in SBC encoding buffer */
  memcpy((uint8_t*)this->pcmBuffer, (uint8_t*)up_sampled_buffer,
         bytes_needed);
  /* update the residue */
  this->feeding_state.aa_feed_residue -= bytes_needed;
...
uint16_t A2dpSbcEncoder::adjust_effective_mtu(
    const tA2DP_ENCODER_INIT_PEER_PARAMS& peer_params) {
...
uint8_t A2dpSbcEncoder::calculate_max_frames_per_packet(void) {
  ...
  this->TxAaMtuSize =
      adjust_effective_mtu(this->peer_params);
  ...
  LOG_VERBOSE("%s: Effective Tx MTU to be considered: %d", __func__,
    ...
    // LOG_INFO("%s: force set freq to 44100", __func__);
    // p_encoder_params->s16SamplingFreq = SBC_sf44100;
    
  LOG_INFO("%s: p_encoder_params->s16SamplingFreq = %d", __func__, p_encoder_params->s16SamplingFreq);
  switch (p_encoder_params->s16SamplingFreq) {
    case SBC_sf44100:
      if (frame_len == 0) {
