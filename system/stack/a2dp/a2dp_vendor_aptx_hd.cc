...
int A2DP_VendorGetBitRateAptxHd(const RawAddress& peer_address,
                                         const uint8_t* p_codec_info) {
...
A2dpEncoderInterface* A2DP_VendorGetEncoderInterfaceAptxHd(
    ...
    const uint8_t* p_codec_info) {
  LOG_DEBUG("%s: peer_address:%s", __func__, peer_address.ToString().c_str());
  if (!A2DP_IsVendorSourceCodecValidAptxHd(p_codec_info)) return NULL;

  A2dpEncoderInterface* encoder =
    (A2dpEncoderInterface*)new A2dpAptxHdEncoder(peer_address);
...
bool A2dpCodecConfigAptxHdSource::init() {
  if (!(isValid() && A2DP_IsCodecSupported(BTAV_A2DP_CODEC_INDEX_SOURCE_APTX_HD))) {
    return false;
  }
...
bool A2dpCodecConfigAptxHdBase::setCodecConfig(const uint8_t* p_peer_codec_info,
  ...
  if (codec_config_.sample_rate == BTAV_A2DP_CODEC_SAMPLE_RATE_NONE) {
    LOG_ERROR("%s: cannot match sample frequency: local caps = 0x%x "
              "peer info = 0x%x",
              __func__, p_a2dp_aptx_hd_cap->sampleRate,
  ...
  if (codec_config_.channel_mode == BTAV_A2DP_CODEC_CHANNEL_MODE_NONE) {
    LOG_ERROR("%s: cannot match channel mode: local caps = 0x%x "
              "peer info = 0x%x",
              __func__, p_a2dp_aptx_hd_cap->channelMode,
...
bool A2dpCodecConfigAptxHdSink::init() {
  ...
  if (A2DP_GetCodecLocation(BTAV_A2DP_CODEC_INDEX_SINK_APTX_HD) !=
    ...
    LOG_INFO("%s: non software decoder", __func__);
    return true;
  }
  ...
  if (!A2DP_VendorAuthenticateAptxHd()) {
    LOG_ERROR("%s: cannot authenticate the decoder", __func__);
    return false;
  }
